...
// Default FPS used for frame number display; overridden by actual video FPS from metadata
...
export default function Tagging() {
  ...
  const [isVideoLoading, setIsVideoLoading] = useState(false); // spinner while buffering
  const [videoFps, setVideoFps] = useState(DEFAULT_FPS); // actual FPS from metadata
  // Track whether there are unsaved changes (separate from log count to avoid
  // false-positive "unsaved" warning after a successful CSV save).
  const [hasUnsavedChanges, setHasUnsavedChanges] = useState(false);
  const videoRef = useRef<HTMLVideoElement>(null);
  const mountedRef = useRef(true);
  const { showToast } = useToast();

  // Block in-app navigation only when there are genuinely unsaved changes.
  ...
  // Warn before browser close/refresh only when there are unsaved changes.
  useEffect(() => {
  ...
  // calculate current frame from time using actual video FPS
  ...
  // When the selected file changes, force the video element to reload.
  // The key prop already remounts the element; this call is a belt-and-suspenders
  // fallback for browsers that reuse the DOM node.
  useEffect(() => {
  ...
  const handleFileSelect = async (filename: string) => {
    ...
    setDuration(0);
    setIsPlaying(false);
    setVideoFps(DEFAULT_FPS); // reset to default until metadata arrives
    ...
    try {
      ...
      // Use recorded FPS if available (more accurate than default)
  ...
  const skipFrames = (frameCount: number) => {
    if (videoRef.current) {
      const delta = frameCount / videoFps;
  ...
  const handleSaveCSV = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Mark as saved so the blocker and beforeunload handler don't trigger
        // when the user navigates away after a successful save.
        setHasUnsavedChanges(false);
      } else {
  ...
  const handleDeleteLog = (logId: number) => {
    setActionLogs((prev) => {
      ...
      // Still unsaved if there are remaining logs; cleared if empty
      ...
      <div className="flex-shrink-0 bg-clinical-card dark:bg-clinical-dark-card border border-clinical-border dark:border-clinical-dark-border rounded p-2">
        {isLoadExpanded ? (
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {/* FPS badge */}
            <span className="px-2 py-0.5 text-xs font-mono rounded bg-clinical-bg dark:bg-clinical-dark-bg border border-clinical-border dark:border-clinical-dark-border text-clinical-text-secondary dark:text-clinical-text-dark-secondary">
              {videoFps} fps
            </span>
            ...
            {hasUnsavedChanges && (
              <span className="px-2 py-0.5 text-xs font-medium rounded bg-clinical-warning/10 text-clinical-warning border border-clinical-warning/30">
                Unsaved
              </span>
            )}
          </div>
