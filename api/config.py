...
def detect_realsense_devices(retry_count=3, retry_delay=1.0):
    """
    Detect connected RealSense devices.

    Args:
        retry_count: Number of times to retry detection (for slow USB init)
    ...
    for attempt in range(retry_count):
        try:
            ...
            if len(device_list) == 0 and attempt < retry_count - 1:
                print(f"[Config] No RealSense devices found, retrying in {retry_delay}s... (attempt {attempt + 1}/{retry_count})")
            ...
            for dev in device_list:
                try:
                    ...
                    print(f"[Config] Error reading RealSense device info: {e}")
...
def refresh_camera_detection():
    ...
    global _detected_realsense
    _detected_realsense = None
    print("[Config] Camera detection cache cleared")
...
# Multi-camera RealSense settings (lower to avoid USB bandwidth issues)
# Two D455 cameras on same USB controller need reduced settings
# D455 supported FPS at 640x480: 5, 15, 30, 60, 90
REALSENSE_MULTI_CAM_WIDTH = 640
REALSENSE_MULTI_CAM_HEIGHT = 480
REALSENSE_MULTI_CAM_FPS = 60  # Try 60 FPS per camera
REALSENSE_SINGLE_CAM_FPS = 60  # 60 FPS for single camera
