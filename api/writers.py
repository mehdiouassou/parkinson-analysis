...
class _BagWriterWrapper:
    """
    Wrapper that provides VideoWriter-like interface for BAG recording.
    BAG recording happens automatically via the pipeline.
    """

    def __init__(self, pipeline):
        self.pipeline = pipeline
        self._opened = pipeline is not None

    def isOpened(self) -> bool:
        return self._opened

    def write(self, frame):
        # BAG records automatically from pipeline, no explicit write needed
        pass

    def release(self):
        if self.pipeline:
            stop_realsense_recording(self.pipeline)
            self.pipeline = None
            self._opened = False


def create_high_quality_writer(filepath: str, frame_size: Tuple[int, int], fps: int,
                                camera_type: str, pipeline=None, serial: str = None):
    """
    Create BAG writer for RealSense recording.

    Only BAG format is supported (no AVI fallback). Returns None writer
    on failure so the caller can handle the error gracefully.

    Args:
        filepath: Base output path (extension will be set to .bag automatically)
        frame_size: (width, height) tuple
        fps: Frames per second
        camera_type: Expected to be CAMERA_TYPE_REALSENSE
        pipeline: Unused (kept for API compatibility)
        serial: RealSense device serial number

    Returns:
        Tuple of (writer, actual_filepath, recording_pipeline)
        writer is None if BAG recording failed.
    """
    bag_filepath = filepath.rsplit('.', 1)[0] + '.bag'

    rec_pipeline, rec_profile = start_realsense_recording(bag_filepath, serial)
    if rec_pipeline:
        return _BagWriterWrapper(rec_pipeline), bag_filepath, rec_pipeline

    print(f"[Writer] BAG recording failed for {bag_filepath}")
    return None, bag_filepath, None
